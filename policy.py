...
class Policy:
    def __init__(self) -> None:
        self.target=None        #用于存储goal，避免多次计算

        self.A_policy=dict()      #记录各个状态对应的policy
        ...
        #               -404: 默认，代表未定义该状态
        # **请使用rlt=self.A_policy.get(condition,-404)，将默认返回设置为-404**

        self.A_rout=None        #A*，用于存储self.A_Star_path_calculate()计算得到的路径的转弯节点
                                #编码为 (row,cow,direction)
                                #最后一个编码为 (goal_row,goal_col)
        self.A_full_path=False  #A*，是否找到了最优到goal的路径（是否被limit打断导致A*未完全跑完）
        self.A_lib=[]           #A*，用于储存fringe的最小堆，编码方式见函数内说明
        self.A_start_condition=None     #A*，记录本轮A*初始化的出发
        self.A_timer=timer(A_TIME_LIMIT)    #A*，用于检测限定时间是否用尽
        self.A_best_cost=-1             #A*，记录找到最优路径在无干扰情况下最少的步数
    ...
    def distance_check_RS(self,house_map,robot_state:RobotState)->int:
        """
        检查方向上还有几格空间
        """
    ...
    def distance_check_Cd(self,house_map,condition)->int:
        """
        检查方向上还有几格空间
        """
    ...
    def distance_check_Pos(self,house_map,position,direction:int)->int:
        """
        检查方向上还有几格空间
        """
    ...
    def simulate_RS(self,house_map,robot_state:RobotState,action:Action)->RobotState:
        """
        用于模拟运行(无噪声), 应该由self.check_err_rate()调用
        不建议用于大量计算
        """
        row_offset,col_offset=DIR_OFFSET[robot_state.direction]
        ...
        if self.Space_check(house_map,new_pos):
            return RobotState(new_row,new_col,new_dir,speed)
        else:
            dist=self.distance_check_RS(house_map,robot_state)
            steps=dist-speed+1
    ...
    def compare_state_RS(self,rs_1:RobotState,rs_2:RobotState)->bool:
        """
        比较rs_1与rs_2的值是否相等
        """
        if isinstance(rs_1,RobotState) and isinstance(rs_2,RobotState):
    ...
    def check_err_rate(self,house_map,robot_state:RobotState,ag)->Action:
        ...
        封装函数, 用于确定err rate
        """
        if not self.compare_state_RS(self.simul_expected,robot_state):
            if DEBUG:
                print("\t[err rate]: err")
    ...
    def direction_to_offset(self,direction:int)->(int,int):
        """
    ...
    def acc_dacc(self,v:int,l:int)->int:
        """
    ...
    def Heuristic(self,position,direction:int=None)->int:
        ...
        if direction!=None:
            ...
            offsets_to_goal=self.pos_or_neg(position[0]-self.target[0]),self.pos_or_neg(position[1]-self.target[1])
        ...
        return self.line_cost(a)+self.line_cost(b)+c
    
    def Space_check(self,house_map,position)->bool:
        """
    ...
    def A_Policy_generation(self)->None:
        """
        使用self.A_rout生成self.A_policy
        """
        #重置self.A_policy
        self.A_policy=dict()
        ...
        for i,node in enumerate(self.A_rout):
            ...
            for dir in range(4):
                ...
                self.A_policy[condition]=desired_direction+10
            ...
            for j in range(totallen+1):
                ...
                for v in range(4):
                    ...
                    self.A_policy[condition]=self.acc_dacc(v,rest)
    ...
    def get_Action_from_policy(self,robot_state:RobotState,condition:(int,int,int,int),policy_source:dict)->Action:
        """
        从self.A_policy中获得Action, 如果未定义, 返回None
    ...
    def A_Star(self,house_map,robot_state:RobotState)->Action:
        """
        general entry of A*
        """
        #查询当前状态的policy
        condition=robot_state.row,robot_state.col,robot_state.speed,robot_state.direction
        act=self.get_Action_from_policy(robot_state,condition,self.A_policy)
        ...
        if act==None:
            ...
            self.A_Policy_generation()
            if self.A_full_path:
                return self.get_Action_from_policy(robot_state,condition,self.A_policy)
            else:
                return Action(-1,0)
        
        #如果没有计算完，就不初始化A*，而是继续计算A*
        if not self.A_full_path:
            if DEBUG:
                print("\t[A*]: continue to cal!")
            self.A_Star_path_calculate(house_map)
            if self.A_full_path:
                #如果计算完成，就直接返回
                self.A_Policy_generation()
    ...
    def A_Star_path_init(self,house_map,robot_state:RobotState)->None:
        """initiate A*, 并且设置self.A_start_condition=robot_state"""
        ...
        heapq.heappush(self.A_lib,(0+h_0,h_0,robot_state.row,robot_state.col,d_0,0,condition))
        heapq.heappush(self.A_lib,(1+h_1,h_1,robot_state.row,robot_state.col,d_1,1,condition))
        heapq.heappush(self.A_lib,(1+h_2,h_2,robot_state.row,robot_state.col,d_2,1,condition))
        heapq.heappush(self.A_lib,(2+h_3,h_3,robot_state.row,robot_state.col,d_3,2,condition))
    ...
    def A_Star_path_calculate(self,house_map)->None:
        ...
        while True:
            ...
            if (crr[2],crr[3])==self.target:
                ...
                rout.append(self.target)
                rout.append(crr[6])
                self.A_best_cost=crr[5]
                if DEBUG:
                    print(f"\t[A*]: \t\ttarget found! best step:{self.A_best_cost}")
                break
            ...
            for i in range(1,100):
                ...
                heapq.heappush(self.A_lib,(new_cost+new_h_0,new_h_0,new_row,new_col,(crr[4]+1)%4,new_cost,crr_condition))
                heapq.heappush(self.A_lib,(new_cost+new_h_1,new_h_1,new_row,new_col,(crr[4]-1)%4,new_cost,crr_condition))
        ...
        if DEBUG:
            ...
            print(f"\t[A*]: \tprocessed {self.A_timer.check()} time")
    ...
    def get_action(self, house_map, robot_state):
        ...
        return self.check_err_rate(house_map,robot_state,self.A_Star)
