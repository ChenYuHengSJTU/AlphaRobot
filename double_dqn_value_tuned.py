...
def soft_update(target_net, value_net, tau):
...
if __name__ == "__main__":
    ...
    model_path = "model/double_dqn_value_tuned_tmp.pth"
    if args.load == 1:
        if os.path.exists(model_path):
            value_net.load_state_dict(torch.load(model_path))
            target_net.load_state_dict(torch.load(model_path))
            print("load model from model/double_dqn_value_tuned_tmp.pth")
    ...
    for e in range(epochs):
        ...
        for state, state_nxt, action, value in tqdm(dataLoader):
            ...
            td_target[:, actions] = value + target_net(state_nxt_map, action_vec)[:, actions]
        ...
        if e % update_epoch == 0:
            ...
            # 软更新
            # for target_param, param in zip(target_net.parameters(), value_net.parameters()):
                # target_param.data.copy_(target_param.data * update_epsilon + param.data * (1 - update_epsilon))
