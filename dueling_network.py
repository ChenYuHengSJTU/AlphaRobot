...
if __name__ == "__main__":
    ...
    advantage_net = DQN(input_dim=input_dim, action_space=6).to(device=device)
    ...
    model_path = "model/dueling_dqn_value_tmp_new.pth"
    if args.load == 1:
        if os.path.exists(model_path):
            value_net.load_state_dict(torch.load(model_path))
            advantage_net.load_state_dict(torch.load(model_path))
            print("load model from model/dueling_dqn_value_tmp_new.pth")
            value_net.to(device=device)
            advantage_net.to(device=device)
        else:
